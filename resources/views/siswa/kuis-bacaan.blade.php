                    ...
                    <div class="shop-cart-top" style="background-color: #14051d;">
                        <div class="row align-items-center">
                            <div class="col-sm-8" style="padding-bottom: 10px;">
                                <div class="shop-cart-left">
                                    <p class="text-sm" style="font-size: 15px;">Pilih kuis yang ingin dikerjakan</p>
                                </div>
                            </div>
                            <div class="col-sm-4 d-flex align-items-center justify-content-end">
                                <form id="btncari" action="{{ route('search') }}" method="GET" class="d-flex">
                        ...
                        <div class="row" style="margin-top: 10px; color: #fff; font-size: 14px;">
                            <div class="col">
                                    ...
                                    Bab {{ $data->bab }} - {{ $data->nama }} (Sampai {{ Carbon\Carbon::parse($data->tenggat)->isoFormat('D MMMM YYYY [Jam ] HH:mm') }})
                                    @if ($data->hasil->isNotEmpty())                                                             
                                        @foreach ($data->hasil as $datu)                      
                                            @if ($data->ubah === 'tidak') 
                                            <p style="text-transform: Capitalize; margin-top: 10px; font-size: 14px; color: #757372;">Soal : {{ $data->bacaan->soal }}</p>
                                            <p style="text-transform: Capitalize; margin-top: 0px; font-size: 14px; color: #757372;">Jawaban Benar : {{ $data->bacaan->jawaban }}</p>
                                            @endif
                                        @endforeach                                        
                                    @endif
                            </div>
                            ...
                            @if ($data->hasil->isNotEmpty())                            
                                @foreach ($data->hasil as $datu)                      
                                    @if ($data->ubah === 'ya')                              
                                    <a href="{{ route('quiz.infocan', ['id' => $data->id]) }}"
                                        class="cmn-btn ml-2 btn-sm float-right" style="text-transform: Capitalize; margin-top: 8px;">Edit</a>
                                    @else                                
                            ...
                            @else
                                <a href="{{ route('quiz.infocan', ['id' => $data->id]) }}"
                                        class="cmn-btn ml-2 btn-sm float-right" style="text-transform: Capitalize; margin-top: 8px;">Kerjakan</a>
                            @endif
                            </div>
                        </div>
                        @if (!$loop->last)
